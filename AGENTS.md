...
- **색은 "무엇인지"(식별)에만 쓴다.** 목록·캘린더 모두 표시 순서대로 8색을 돌려 써서
  이웃 항목이 항상 다른 색이 되게 한다(`--task-*`). 캘린더는 태스크와 Google 일정을
  한 줄에 세워 번호를 매기므로 둘 사이에서도 색이 겹치지 않는다.
  **종류·중요도는 색이 아니라 아이콘으로**: 우선순위=깃발, Google 일정=캘린더 아이콘.
