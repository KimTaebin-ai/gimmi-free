...
src/
  ...
  lib/
    ...
    rag/
      ...
      search.ts             # 코사인 최근접 검색(벡터를 받는다 — 임베딩 없이 테스트 가능)
...
- **홈(`/`)은 성장 대시보드**다. 제품의 관점: 노력 = 시간을 쏟거나 일을 끝낸 게 아니라
  **이전에 할 수 없던 걸 할 수 있게 되는 것**. 그래서 완료 목록을 나열하지 않고,
  ...
  - 세기는 **기록 10 / 블로그 본문 6 / 요약·설명 1**. 본문을 따로 세는 게 중요하다 —
    한때 블로그 글도 요약만 있어 1점이었고, 그래서 두 달치 회고 글이 "Flight to 서울"
    같은 일정과 같은 순위로 프롬프트 뒤편에 밀렸다.
  - 블로그는 `summary`가 아니라 **`content`(본문)를 프롬프트에 넣는다**(최대 6000자).
    200자 요약으로는 무엇을 이해했는지 알 수 없다.
- **요약 생성은 백그라운드로 돈다.** 버튼을 누른 요청은 `GrowthJob` 행만 만들고 바로 응답하고,
  실제 생성은 `after()`가 응답 뒤에 이어서 한다. 그래서 화면을 옮기거나 새로고침해도 계속된다.
  - 진행 상태는 **클라이언트가 아니라 DB**에 있다. 그래야 새로고침해도 "정리하는 중"이 이어진다.
    화면은 도는 동안만 3초 간격으로 물어본다.
  - 이미 도는 작업이 있으면 새로 만들지 않는다(두 번 눌러도, 탭이 두 개여도 한 번만 과금).
  - 서버리스 함수는 시간이 다 되면 통보 없이 죽는다. 그러면 작업이 영원히 running으로 남으므로
    **상태를 물어볼 때마다** 오래된 작업(10분)을 실패로 정리한다. Hobby 크론은 하루 한 번이라
    크론에 맡기면 화면이 하루 종일 돌아간다.
  - 홈 라우트에 `maxDuration = 300`을 둔 건 `after()`가 끝날 시간을 벌기 위해서다.
- LLM 호출은 **사용자가 버튼을 누를 때만** 한다(화면 로딩만으로 과금되지 않도록).
  ...
  - **월별 타임라인**은 저장된 요약을 겹쳐 만든다. 규칙은 하나다 —
    **한 달의 내용은 그 달을 다룬 가장 나중 요약의 것만 쓴다.**
    ...
    (실제로 "공학 문제를 최적화 문제로 환원…"과 "SLAM·로보틱스 문제를 최적화 문제로 환원…"이
    한 달에 나란히 남았다). 그래서 항목이 아니라 **요약 단위로** 고른다.
    능력마다 `month`("YYYY-MM")는 모델이 근거 날짜를 보고 붙인다.
...
- **검색은 기간으로 자르지 않는다.** 글을 두 달에 한 편 쓰면 90일 창에 한 편밖에
  안 들어와 검색할 코퍼스가 사라진다(실제로 19편 중 1편만 대상이었다). 대신
  `balanceByPeriod`로 기간 안 대목을 먼저 채우고 기간 밖은 8개까지만 곁들이며,
  발췌마다 "이 기간 이전에 쓴 글" 표시를 붙인다 — 예전 글은 이번 기간의 성장이 아니라
  **"이건 예전부터 하던 것"을 가려내는 근거**로 쓰라고 프롬프트에 적어 두었다.
- 임베딩 호출은 **사용자가 "정리해 줘"를 누를 때만** 일어난다(화면 로딩엔 없음).
  Claude 호출 규칙과 같은 이유다.
- **탐침은 한 요청으로 모아 임베딩한다**(`embedQueries`). 탐침마다 부르면 요청이 10번 나가
  속도 제한에 걸려 버튼 한 번이 몇 분씩 걸린다. 그 뒤 검색은 DB만 쓰므로 탐침이 늘어도 공짜다.
- **유사도 문턱값(`MIN_EXCERPT_SCORE`)은 낮게 잡는다(0.15).** 추상적인 탐침 문장과 구체적인
  서술 문단 사이의 코사인 유사도는 실측상 0.2~0.45다. 0.35처럼 "그럴듯한" 값을 넣으면
  발췌가 통째로 비어 RAG가 조용히 아무 일도 안 한다. 순위는 점수가, 개수는 상한이 맡는다.
- Voyage 계정에 **결제수단이 없으면 분당 3회 / 1만 토큰**으로 묶인다(무료 사용량과는 별개).
  그래서 색인은 429를 만나면 기다렸다 재시도하고, 요청이 토큰 한도보다 크면 배치를 쪼갠다.
  그래도 못 끝내면 시간 예산(40초)에서 멈추고 남은 글을 다음 동기화로 넘긴다.
